            ...
            if base is not None:
                for v in base.split(","):
                    if not v:
                        continue
                    base = v
            add = request.POST.get('add_localcompany_number')
            if add is not None:
                for v in add.split(","):
                    # info(v)
                    if not v:
                    ...
                    if rec:
                        ...
                        if obj.first() is not None:
                            obj = IndexLocalCompany.objects.filter(local_company_id=v,
                                                                   index_id=change_id).get()
                ...
                for v in delete.split(","):
                    ...
                    if rec:
                        ...
                        if obj.first() is not None:
                            obj = IndexLocalCompany.objects.filter(local_company_id=v,
                                                                   index_id=change_id).get()
                            IndexLocalCompany.objects.get(id=obj.id).delete()
                        else:
                            b = IndexLocalCompany.objects.get(id=obj.id)
                            b.add_flg = 2
                            b.save()
            ...
            for key, condition in conditions.items():
                ...
                if 'contract_termination_flag' in condition:
                    if condition['contract_termination_flag'] == 'true':
                        ...
                        if sub_queries[9] is None:
                            sub_queries[9] = Q()
                        sub_queries[9] |= Q(
                            contract_termination_flag__iexact=True)
                        add_condition(order_disp, '終了フラグ', 'チェック')
                ...
                if 'hidden_flag' in condition:
                    if condition['hidden_flag'] == 'true':
                        ...
                        sub_queries[13] |= Q(hidden_flag__iexact=True)
                        order_disp['非開示'] = 'チェック'
                    ...
                    if 'original_storage_location_exclude' in condition:
                        if condition['original_storage_location_exclude'] == 'None':
                            ...
                            if temp_queries[15] is None:
                                temp_queries[15] = Q()
                            temp_queries[15] |= ~Q(
                                original_storage_location_iexact=condition['original_storage_location_exclude'])
                            add_condition(
                ...
                if 'local_company_number_total' in condition and condition['local_company_number_total']:
                    ...
                    sub_queries[17] &= Q(
                        index__add_flg=0) | Q(index__add_flg=1)
