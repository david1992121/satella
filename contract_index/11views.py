        ...
        if conditions is not None:
            ...
            for cc_list in per_dup:
                ...
                if len(cc_list) > 0:
                    ...
                    for cc in cc_list:
                        if cnt == 0:
                            mid_q &= Q(contract_companies__startswith=cc_list[0][0]+'/')
                        elif cnt == len(cc_list) - 1:
                            mid_q &= Q(contract_companies__endswith='/'+cc_list[-1][0])
                        elif cnt < len(cc_list) - 1 and cnt != 0:
                            mid_q &= Q(contract_companies__icontains='/'+cc_list[cnt][0]+'/')
                        query += str(cc[0]) + '/'
                        cnt += 1
                    query = query[:-1]
