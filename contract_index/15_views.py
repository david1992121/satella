...
def searchresult(request):
    ...
    if 'special_search' in request.POST:
        ...
        if conditions is not None:
            ...
            for key, condition in conditions.items():
                ...
                if 'contract_companies' in condition and condition['contract_companies']:
                    ...
                    if 'contract_companies_checkbox' in condition:
                        ...
                        dup_cc.append([keyword, 'comp'])
                        add_condition(order_disp, '契約当事者(完全一致)', condition['contract_companies'])
                    else:
                        ...
                        #     temp_queries[2] = Q()
                        for w in word:
                            dup_cc.append([w, 'part'])
                            # temp_queries[2] &= Q(contract_companies__icontains=w)

                        add_condition(order_disp, '契約当事者', ','.join(word) + search_condition)
                ...
                # 光通信Grpの債務保証有無の検索
                if request.user.has_perm('contract_index.loan_guarantee_availability'):
                ...
                # 原本保管場所の検索
                if request.user.has_perm('contract_index.original_storage_location'):
                ...
                if 'local_company_number_original' in condition and condition['local_company_number_original']:
                    ...
                    for w in word:
                        temp_queries[16] |= Q(
                            index__local_company_id=int(w.replace(',', '')))
                ...
                if 'local_company_number_total' in condition and condition['local_company_number_total']:
                    ...
                    if temp_queries[17] is None:
                        temp_queries[17] = Q()
                    for w in word:
                        temp_queries[17] |= Q(index__local_company_id=int(w.replace(',', '')))
                ...
                if 'local_company_number_add' in condition and condition['local_company_number_add']:
                    ...
                    if temp_queries[20] is None:
                        temp_queries[20] = Q()
                    for w in word:
                        temp_queries[20] |= Q(index__local_company_id=int(w.replace(',', '')))
            ...
            cc_query, part_query, comp_query = Q(), Q(), Q()
            for cc_list in per_dup:
                ...
                if len(cc_list) > 0:
                    ...
                    for cc in cc_list:
                        if cc[1] == 'comp':
                            comp_q = Q()
                            comp_dup, comp_cnt = 0, 0
                            for c in cc_list:
                                if c[0] == cc[0]:
                                    comp_cnt += 1
                                if comp_cnt > 1:
                                    comp_q &= Q(contract_companies__icontains='//'+cc[0]+'//'+cc[0]+'//') | (Q(contract_companies__icontains='//'+cc[0]+'//') & Q(contract_companies__endswith='//'+cc[0]+'/')) | (Q(contract_companies__icontains='//'+cc[0]+'//') & Q(contract_companies__startswith='/'+cc[0]+'//'))
                                    # comp_q &= Q(contract_companies__icontains='/'+cc[0]+'/'+cc[0]+'/') | (Q(contract_companies__icontains='/'+cc[0]+'/') & Q(contract_companies__endswith='/'+cc[0])) | (Q(contract_companies__icontains='/'+cc[0]+'/') & Q(contract_companies__startswith=cc[0]+'/'))
                                    comp_dup = 0
                                    comp_query &= comp_q
                                    break
                                    ...
                                    continue
                            if comp_dup == 1:
                                mid_q &= (Q(contract_companies__icontains='//'+cc_list[cnt][0]+'//') | Q(contract_companies__endswith='//'+cc_list[cnt][0]+'/') | Q(contract_companies__startswith='/'+cc_list[cnt][0]+'//'))
                            else:
                                mid_q &= comp_q
                        elif cc[1] == 'part':
                            part_q = Q()
                            part_dup = 0
                            for c in cc_list:
                                if c[1] == 'comp' and c[0] == cc[0]:
                                    cc_pp_list = []
                                    cc_pp = Index.objects.filter(contract_companies__icontains=cc[0]).values_list('contract_companies')
                                    for pp in cc_pp:
                                        company_part = pp[0].split('/')
                                        company_part = filter(None, company_part)
                                        for cp in company_part:
                                            if cc[0] in cp:
                                                cc_pp_list.append(cp)
                                        cc_pp_list = list(set(cc_pp_list))
                                    for cp in cc_pp_list:
                                        if cp != cc[0]:
                                            part_q |= ((Q(contract_companies__icontains='//'+cp+'//') | Q(contract_companies__endswith='//'+cp+'/') | Q(contract_companies__startswith='/'+cp+'//')) & (Q(contract_companies__icontains='//'+cc[0]+'//') | Q(contract_companies__endswith='//'+cc[0]+'/') | Q(contract_companies__startswith='/'+cc[0]+'//')))
                                    part_query &= part_q
                                    break
                                else:
                                    part_dup = 1
                            ...
                            if part_dup == 1:
                                mid_q &= Q(contract_companies__icontains=cc[0])
                            else:
                                mid_q &= part_q
                        query += '/' + str(cc[0]) + '/'
                        # query += str(cc[0]) + '/'
                        cnt += 1
                    query = query[:-1]
                cc_query |= Q(contract_companies__icontains=query) | mid_q
                cc_query &= part_query
                cc_query &= comp_query
